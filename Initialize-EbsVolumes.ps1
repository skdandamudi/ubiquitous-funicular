...
function Get-FreeDriveLetters {
    ...
    $used += (Get-CimInstance Win32_NetworkConnection -ErrorAction SilentlyContinue).LocalName |
                ForEach-Object { if ($_) { $_.TrimEnd(':') } }
...
foreach ($disk in $candidates) {
    ...
    if ($freeLetters.Count -eq 0) {
        ...
        $results += [pscustomobject]@{ Disk = $disk.Number; Drive = $null; SizeGB = $sizeGB; Status = 'NoLetterAvailable' }
        continue
    }
    ...
    if (-not $PSCmdlet.ShouldProcess($tag, "Initialize GPT, format $FileSystem, assign ${letter}:")) {
        ...
        $results += [pscustomobject]@{ Disk = $disk.Number; Drive = "${letter}:"; SizeGB = $sizeGB; Status = 'WhatIf' }
        continue
    }
